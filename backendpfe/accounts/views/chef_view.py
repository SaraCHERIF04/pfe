...
from ..models import Employe, Projet, SousProjet, Reunion, Incident, Utilisateur, Document, DocumentFile,Chefprojet
from ..permissions import IsChefDeProjet
...
class ChefView(APIView):
    ...
    def get_chef(self, request):
        ...
        try:
            return Chefprojet.objects.get(id_utilisateur=request.user.id_utilisateur)
    ...
    def get(self, request, data_type=None):
        ...
        """
        chef = self.get_chef(request)
        if not chef:
            return Response({
                'success': False,
                'message': 'Chef de projet record not found for this user'
            }, status=status.HTTP_404_NOT_FOUND)
            
        if data_type == 'projects':
            return self.get_projects(request, chef)
        elif data_type == 'subprojects':
            return self.get_subprojects(request, chef)
        elif data_type == 'reunions':
            return self.get_reunions(request, chef)
        elif data_type == 'incidents':
            return self.get_incidents(request, chef)
        elif data_type == 'documents':
            return self.get_documents(request, chef)
        else:
            return self.get_dashboard(request, chef)
    
    def get_dashboard(self, request, chef):
        """
        Get all projects, subprojects, reunions, and incidents associated with the logged-in chef de projet.
        """
        try:
            # Get projects where this chef is assigned
            projects = Projet.objects.filter(id_utilisateur=chef)
            
            # Get subprojects where this chef is assigned
            ...
            if not projects and not subprojects:
                return Response({
                    'success': True,
                    'message': 'No projects or subprojects assigned to this chef de projet',
                    'data': {
                        'projects': [],
                        'subprojects': [],
                        'reunions': [],
            ...
            # Get all reunions related to the chef's projects
            reunions = []
            if projects:
                project_ids = [p.id_projet for p in projects]
                reunions = Reunion.objects.filter(id_projet__in=project_ids).distinct()
            
            # Get all incidents related to the chef's projects/subprojects
            incidents = []
            ...
            return Response({
                'success': True,
                'message': 'Chef de projet data retrieved successfully',
                'data': {
    ...
    def get_projects(self, request, chef):
        ...
        try:
            # Get projects where this chef is the chef de projet
            projects = Projet.objects.filter(id_utilisateur=chef)
    ...
    def get_subprojects(self, request, chef):
    ...
    def get_reunions(self, request, chef):
        """Get reunions related to the chef's projects"""
        try:
            # Get reunions where this chef is the chef de projet or from projects managed by this chef
            reunions = Reunion.objects.filter(
                Q(id_utilisateur=chef) | 
                Q(id_projet__id_utilisateur=chef)
            ).distinct()
    ...
    def get_documents(self, request, chef):
        """Get all documents associated with the chef's projects and subprojects"""
        try:
            # Get projects and subprojects managed by this chef
            projects = Projet.objects.filter(id_utilisateur=chef)
            subprojects = SousProjet.objects.filter(id_projet__id_utilisateur=chef)
            ...
            if not projects and not subprojects:
                return Response({
                    'success': True,
                    'message': 'No projects or subprojects assigned to this chef, thus no documents available',
                    'data': []
                }, status=status.HTTP_200_OK)
            
            # Get all documents related to the chef's projects/subprojects
            q_objects = Q()
            if projects:
                project_ids = [p.id_projet for p in projects]
                q_objects |= Q(id_projet__in=project_ids)
    ...
    def post(self, request, data_type=None):
        ...
        if data_type == 'documents':
            return self.add_document(request, chef)
        else:
            return Response({
                'success': False,
    ...
    def put(self, request, document_id=None):
        ...
        if document_id:
            return self.edit_document(request, chef, document_id)
        else:
            return Response({
                'success': False,
                'message': 'Invalid endpoint'
    ...
    def add_document(self, request, chef):
        """Add a new document associated with the chef's project or subproject"""
        try:
            data = request.data
            ...
            # Verify that the chef has access to the specified project/subproject
            project = None
            subproject = None
            ...
            if 'subproject_id' in data and data['subproject_id']:
                try:
                    ...
                    # Check if chef is the chef de projet for this subproject
                    subproject = SousProjet.objects.get(id_sous_projet=subproject_id, id_utilisateur=chef)
                    if not subproject:
                        return Response({
                            'success': False,
                    ...
                    return Response({
                        'success': False,
                        'message': 'Subproject not found or you do not have access to it'
                    }, status=status.HTTP_404_NOT_FOUND)
    ...
    def edit_document(self, request, chef, document_id):
        ...
        try:
            ...
            # Check if the chef has access to the document's project/subproject
            has_access = False
            
            # Check if chef is assigned to the document's project
            if document.id_projet:
                try:
                    Projet.objects.get(id_projet=document.id_projet.id_projet, id_utilisateur=chef)
                    has_access = True
                except Projet.DoesNotExist:
                    pass
                
            # Check if chef is assigned to the document's subproject
            if document.id_sous_projet and not has_access:
                try:
                    SousProjet.objects.get(id_sous_projet=document.id_sous_projet.id_sous_projet, id_utilisateur=chef)
                    has_access = True
                except SousProjet.DoesNotExist:
                    pass
